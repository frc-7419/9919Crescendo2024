  ...
  public void revWhenInRange() throws InterruptedException {
    ...
    if (currentX <= lineXValue) {
      ...
      if (toggleAutoRev) {
        ...
        // Wait for the shooter to rev up; the time may need to be adjusted based on testing
        long revTime = 4; // seconds
        TimeUnit.SECONDS.sleep(revTime);
        revingTime += revTime;
      }
    } else {
      inRange = false;
